...
class DjangoSandboxConfig(AppConfig):
    ...
    def ready(self):
        ...
        * Every existing Sandbox have a period task for "poll usage" and
        ...
        """
        # Check settings
        if not isinstance(settings.SANDBOX_POLL_USAGE_EVERY, int) or settings.SANDBOX_POLL_USAGE_EVERY < 15:
            raise ValueError(f"Incorrect SANDBOX_POLL_USAGE_EVERY settings:{settings.SANDBOX_POLL_USAGE_EVERY}")
        if not isinstance(settings.SANDBOX_POLL_SPECS_EVERY, int) or settings.SANDBOX_POLL_SPECS_EVERY < 300:
            raise ValueError(f"Incorrect SANDBOX_POLL_SPECS_EVERY settings:{settings.SANDBOX_POLL_SPECS_EVERY}")
        
        # Load needed models
        PeriodicTask = apps.get_model(app_label='django_celery_beat', model_name='PeriodicTask')
        IntervalSchedule = apps.get_model(app_label='django_celery_beat', model_name='IntervalSchedule')
        ...
        PeriodicTask.objects.filter(name__startswith="sandbox_poll_usage").update(interval=usage_schedule.pk)
        PeriodicTask.objects.filter(name__startswith="sandbox_poll_specifications").update(interval=specs_schedule.pk)
        ...
        for sandbox in Sandbox.objects.all():
            ...
            try:
                ...
                PeriodicTask.objects.create(
                    name=task_name, interval=specs_schedule, task='django_sandbox.tasks.poll_specifications',
                    args=json.dumps([sandbox.pk])
                )
