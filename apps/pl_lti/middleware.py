...
from pl_lti.models import ActivityOutcome
...
class LTIAuthMiddleware(MiddlewareMixin):
    ...
    monkey-patching of django's reverse() function (see ./__init__.py)
    can access it in order to retrieve the current resource_link_id.
    """
    ...
    def process_request(self, request):
        ...
        if isPost and request.POST.get('lti_message_type') == 'basic-lti-launch-request':
            ...
            if user is not None:
                ...
                if urlmatch and urlmatch.app_name + ":" + urlmatch.url_name == "activity:play":
                    # TODO récupérer ou créer une activity
                    logger.info('LTI authentication OK')
            else:
